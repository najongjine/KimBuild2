...
var showUpdateCmtForm=function(bcmtSeq){
	...
	let updateCmtFormHtml=`
		...
		<td >
			<a href="#" onclick="submitUpdateCmt('\${bcmtSeq}')" class="btn btn_middle btn_purple" >수정</a> 
			<a href="#" onclick="getCmtList()" class="btn btn_middle btn_gray" >취소</a>
		</td>
	`
				...
				<c:when test="${fn:length(resultList) > 0}">
					<c:forEach var="result" items="${resultList}" varStatus="status">
						<tr id="cmtTr_${result.bcmtSeq}" class="cursor">
							<td id="cmtTd_${result.bcmtSeq}" class="subject" ><pre><c:out value="${result.cont}"/></pre></td>
							<td >${result.name }</td>
							<td >${result.rgstDt }</td>
							<td >
								<c:if test="${result.rgstId eq loginFtSeq}">
									<a href="#" onclick="showUpdateCmtForm('${result.bcmtSeq}')" class="btn btn_middle btn_purple" >수정</a> 
									<a href="#" onclick="deleteCmt(${result.bcmtSeq})" class="btn btn_middle btn_red" >삭제</a>
								</c:if>
							</td>
						</tr>
					</c:forEach>
